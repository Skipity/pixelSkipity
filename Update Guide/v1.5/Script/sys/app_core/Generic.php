	...
	public static function PX_LoadPage($page_url = '', $data = array(), $set_lang = true) {
	    ...
	    if ($set_lang == true) {
	        $page_content = preg_replace_callback("/{{LANG (.*?)}}/", function($m) use ($lang_array) {
				$lang = '';
				if( isset( $lang_array[$m[1]] ) ){
					$lang = $lang_array[$m[1]];
				}else{
					// $keyd = trim(strtolower(preg_replace('/[^a-zA-Z0-9-_\.]/','_', $m[1])));

					// $exist = self::$db->where('lang_key', $keyd )->getValue(T_LANGS,'count(*)');
					// if( $exist === 0 ){
					// 	self::$db->insert(T_LANGS,array(
					// 		'lang_key' => $keyd,
					// 		'english' => $m[1]
					// 	));
					// }
