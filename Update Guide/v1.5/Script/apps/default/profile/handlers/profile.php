...
if ($config['private_photos'] == 'on' || $config['private_photos'] == 'on') {
	$month = 60 * 60 * 24 * 30;
...
elseif ($page == 'reels' && $navbar ) {
	$posts->setUserById($user_id);
	$posts->limit = 6;
	// $context['content_page'] = "reels.html";
	$user_posts   = $posts->getUserReels();
	$context['posts'] = o2array($user_posts);
	$context['page_link'] = $_GET['uname'].'/reels';

}
elseif ($page == 'subscriptions' && ($config['private_photos'] == 'on' || $config['private_photos'] == 'on') && ($context['user_data']['show_subscribers'] == 1 || ($context['user_data']['show_subscribers'] != 1 && $is_owner))) {
	$month = 60 * 60 * 24 * 30;
